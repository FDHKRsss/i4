...
- Test the real `navigator.geolocation` wrapper in jsdom by stubbing `window.navigator.geolocation` (via `Object.defineProperty(..., {value, configurable:true, writable:true})`) whose `getCurrentPosition(ok, err, opts)` records `opts` and calls `ok`/`err`. Assert the requested `{enableHighAccuracy:true, timeout:10000, maximumAge:0}`; drive the browser-error-code mapping (1→permission-denied, 2→position-unavailable, 3→timeout, else unknown) with `it.each`; reset `navigator.geolocation` to `undefined` in `afterEach`.
- Pin documentation state with a spec so doc drift fails the suite instead of waiting for the critic:
