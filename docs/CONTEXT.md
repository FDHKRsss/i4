...
- **M12 (review/submit) and M13 (reports list) are done frontend-first against the M14 backend contract.** The seed backend still expects the old `voice`/`audio_path` row shape, so a real `POST /api/report` or `GET /api/reports` will fail until **M14** lands. Keep their mocked-`fetch` tests; do **not** rewrite the backend before M14.
